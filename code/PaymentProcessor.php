...
class PaymentProcessor extends Controller {
	 ...
	 * @param String $url
	 */
	public function setRedirectURL($url, $session) {
	 ...
	 * @return String
	 */
	public function getRedirectURL($session) {
	 ...
	 */
	public function capture($data, $session) {
...
class PaymentProcessor_GatewayHosted extends PaymentProcessor {
	...
	public function capture($data, $session) {
		...
		$this->gateway->callbackURL = Director::absoluteURL('home/fprn');
	...
	public function complete($request) {
		...
		// Do redirection
		$this->doRedirect($request->getSession());
	}

	private function paymentTxnIDMatchesGatewayTxnID() {
		if (method_exists($this->gateway, 'getTxnID')) {
			return ($this->payment->TxnID === $this->gateway->getTxnID());
		} else {
